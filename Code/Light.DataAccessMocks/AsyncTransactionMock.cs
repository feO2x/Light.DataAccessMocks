...
/// </summary>
public sealed class AsyncTransactionMock : AsyncDisposableMock<AsyncTransactionMock>,
                                           IAsyncTransaction,
                                           ITransactionMock
{
    /// <summary>
    ...
    public AsyncTransactionMock MustBeCommitted()
    {
        if (CommitCallCount != 1)
        {
            throw new TestException(
                $"CommitAsync must have been called exactly once, but it was called {CommitCallCount} times."
            );
        }
